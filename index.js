...
async function renderIcons() {
    ...
    const itemIds = icons.map((element) => (element.attributes['data-item-id'].value)).filter((elem) => (elem && elem != ''));
    if (itemIds.length == 0) return;
    const items = await api.items().many(itemIds);
    items.forEach(({ id, icon, rarity, name }) => {
        const itemContainers = document.querySelectorAll(`.item-unrendered.item-${id}`);
        const itemImages = document.querySelectorAll(`.item-unrendered.item-${id} .item-icon`);
        const itemNames = document.querySelectorAll(`.item-unrendered.item-${id} .item-name`);
        itemContainers.forEach((container) => (container.classList.add(rarity.toLowerCase())));
        itemImages.forEach((image) => (image.src = icon));
        itemNames.forEach((itemName) => (itemName.innerHTML = name));
    });
...
terminal.renderItem = function(itemId, count = -1, name = '') {
