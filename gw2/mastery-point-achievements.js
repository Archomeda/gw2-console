...
async function masteryPointAchievements() {
    const achievements = await _api.achievements().all();
    ...
    const [accountAchievements, accountPoints] = await Promise.all([
        _api.account().achievements().get(),
        _api.account().mastery().points().get()
    ]);
    ...
    _terminal.writeLine(`<span style='color:yellow;'>This command will verify your account for all locked and unlocked mastery points. Please wait...</span>`);
    _terminal.writeLine();
...
module.exports = (terminal, api) => {
    _terminal = terminal;
    _api = api;
    return masteryPointAchievements;
};
